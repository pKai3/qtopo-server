...
async function renderVector(job) {
  ...
  const context = { controllers: new Set(), error: null };
  if (!retained) {
...
async function run(job) {
  ...
  if (job.kind === 'composite') {
    ...
    const pixels = await renderVector(job);
  ...
  await atomicWrite(job.outPath, canvas.toBuffer('image/png', { compressionLevel: 3 }));
...
process.on('message', async job => {
  ...
  busy = true;
  try {
    await run(job); busy = false;
    if (process.connected) process.send({ done: true });
  } catch (err) {
