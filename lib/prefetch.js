...
function createPrefetch({ radius = 1, zoom = true, concurrency = 1, limit = 64, delayMs = 500, zoomRange = () => ({}), render, logger }) {
  ...
  function schedule() {
    if (closed || !radius || timer || active.size >= concurrency || requests || !pending.size) return;
    timer = setTimeout(drain, Math.max(0, quietUntil - Date.now())); timer.unref();
  }
  function drain() {
    timer = null;
    if (closed || requests || Date.now() < quietUntil) return schedule();
    while (active.size < concurrency && pending.size) {
      ...
      if (job.priority === PRIORITY.ZOOM && [...active.values()].some(task => task.priority === PRIORITY.NEIGHBOUR)) break;
  ...
  return {
    ...
    get stats() {
      return {
        ...
        active: active.size, queued: pending.size, completed, completedZoom,
