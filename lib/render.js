...
const { createTimings } = require('./metrics');
function createRenderer(config, workerPath = path.join(__dirname, 'render_worker.js')) {
  ...
  const timingsSummary = createTimings(['queueMs', 'workerMs', 'resourceWaitMs', 'encodeMs', 'cpuMs']);
  ...
  async function finish(worker, error, timings) {
    ...
    try {
      ...
      if (timings) {
        ...
        timingsSummary.record(lastRender);
  ...
  function render(job) {
    if (inflight.has(job.outPath)) {
      const existing = inflight.get(job.outPath);
      existing.job.priority = Math.min(existing.job.priority ?? PRIORITY.REQUEST, job.priority ?? PRIORITY.REQUEST);
      existing.job.rank = Math.max(existing.job.rank || 0, job.rank || 0);
    ...
    const p = queue.run(async () => {
      ...
      return new Promise((resolve, reject) => {
        ...
        worker.child.send({ ...job, pngCompression: config.pngCompression, timeoutMs: config.upstreamTimeoutMs, resourceMaxAgeMs }, err => { if (err) finish(worker, err); });
      });
    }, { priority: job.priority, key: job.outPath, rank: job.rank }).finally(() => inflight.delete(job.outPath));
    inflight.set(job.outPath, { promise: p, job }); return p;
  }
  return {
    render,
    get stats() { return { ...queue.stats, workers: workers.size, spawned, completed, lastRender, timings: timingsSummary.stats }; },
    close() {
      queue.close();
