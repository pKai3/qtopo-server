...
const { httpError, cachedFile } = require('./utils');
function createRenderer(config) {
  ...
  function render(job) {
    ...
    const p = queue.run(async () => {
      const cached = await cachedFile(job.outPath, config.rasterTTL);
      if (cached?.size) return job.outPath;
      await fs.mkdir(path.dirname(job.outPath), { recursive: true });
      return new Promise((resolve, reject) => {
        let workerError;
        const child = fork(path.join(__dirname, 'render_worker.js'), [], { stdio: ['ignore', 'inherit', 'inherit', 'ipc'] });
        children.add(child);
        ...
        child.on('close', async code => {
          ...
          try {
            if (!(await cachedFile(job.outPath, config.rasterTTL))?.size) throw httpError('Renderer produced no output');
            resolve(job.outPath);
