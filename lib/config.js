...
function loadConfig(env = process.env) {
  ...
  const tilePx = number(env, 'TILE_PX', 512, 256, 512, true);
  if (![256, 512].includes(tilePx)) throw new Error('TILE_PX must be 256 or 512');
  ...
  return {
    ...
    renderConcurrency: number(env, 'RENDER_CONCURRENCY', 2, 1, 16, true),
