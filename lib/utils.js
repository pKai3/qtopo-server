...
async function cachedRaster(file, ttl, emptyTTL) {
  const marker = await cachedFile(file + '.empty', 0);
  const stat = await cachedFile(file, marker ? emptyTTL : ttl);
  return stat?.size ? { ...stat, empty: !!marker } : null;
}
...
module.exports = { atomicWrite, cachedFile, cachedRaster, httpError, imageType };
