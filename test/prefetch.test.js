...
const { createPrefetch, neighbours } = require('../lib/prefetch');
...
test('prefetch waits for requests, stays bounded, deduplicates and never expands recursively', async t => {
  const seen = [];
  const prefetch = createPrefetch({ radius: 1, limit: 8, delayMs: 1, render: async (provider, tile) => seen.push(`${provider}/${tile.z}/${tile.x}/${tile.y}`) });
